...
struct Aabb {
    ...
    y_range: (i32, i32),
    z_range: (i32, i32),
    children: Vec<Aabb>, // non-overlapping
}
...
impl Aabb {
    ...
    fn does_intersect(&self, other: &Aabb) -> bool {
        if self.x_range.0 <= other.x_range.1 && self.x_range.1 >= other.x_range.0 {
            if self.y_range.0 <= other.y_range.1 && self.y_range.1 >= other.y_range.0 {
                if self.z_range.0 <= other.z_range.1 && self.z_range.1 >= other.z_range.0 {
                    return true;
                }
            }
        }
    ...
    fn intersect(&self, other: Aabb) -> (Option<Aabb>, Vec<Aabb>)
    {
        ...
        //let start_vol = other.calc_volume();
        ...
        if other.y_range.0 < self.y_range.0 {
            ...
            new.y_range.1 = self.y_range.0 - 1;
            remaining.push(new);
        }
        ...
        if other.z_range.0 < self.z_range.0 {
            ...
            let mut new = other.clone();
            new.x_range = intersection.x_range;
            new.y_range = intersection.y_range;
            new.z_range.1 = self.z_range.0 - 1;
        ...
        if other.z_range.1 > self.z_range.1 {
            ...
            new.x_range = intersection.x_range;
            new.y_range = intersection.y_range;
            new.z_range.0 = self.z_range.1 + 1;
    ...
    fn insert(&mut self, aabb: Aabb) -> Vec<Aabb> {
        let (intersect, remaining) = self.intersect(aabb);

        if let Some(intersect) = intersect {
            let mut to_insert = vec![intersect];

            for child in &mut self.children {
                for inserting in to_insert.drain(..).collect::<Vec<Aabb>>() {
                    to_insert.extend(child.insert(inserting));
                }
            }

            self.children.extend(to_insert);
        }
...
fn day22(aabbs: Vec<Aabb>) -> usize {
    let mut root_aabb = Aabb {
        on: false,
        x_range: (-50, 50),
        y_range: (-50, 50),
        z_range: (-50, 50),
...
fn day22_2(aabbs: Vec<Aabb>) -> usize {
    let mut root_aabb = Aabb {
        on: false,
        x_range: (-500000, 500000),
        y_range: (-500000, 500000),
...
on x=10..10,y=10..10,z=10..10";
    ...
    let aabbs = read_input(input);

    assert_eq!(day22(aabbs), 39);
}
...
off x=-93533..-4276,y=-16170..68771,z=-104985..-24507";
    ...
    assert_eq!(day22_2(aabbs), 2758514936282235);
}
...
fn day22_2_actual() {
    ...
    assert_eq!(day22_2(input), 1134725012490723);
}
...
fn day22_aabb_intersect() {
    ...
    assert_eq!(remainder.iter().map(|x| x.calc_volume()).sum::<usize>(), 26);
}
...
fn day22_aabb_intersect2() {
    ...
    dbg!(&remainder);
    let intersect_vol = intersect.unwrap().calc_volume();
    assert_eq!(intersect_vol, 8);
    assert_eq!(intersect_vol + remainder_vol, aabb2.calc_volume());
}
